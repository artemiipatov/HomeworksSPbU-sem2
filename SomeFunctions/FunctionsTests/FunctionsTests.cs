...
public class Tests
{
    ...
    {
        Assert.AreEqual(SomeFunctions.SomeFunctions.Map<int>(new List<int>() {1, 2, 3, 4, 5, 6}, x => x * 2),
            new List<int>() {2, 4, 6, 8, 10, 12});
        Assert.AreEqual(SomeFunctions.SomeFunctions.Map<string>(new List<string>() {"a", "b", "c", "d", "e"}, x => x + "1"),
            new List<string>() {"a1", "b1", "c1", "d1", "e1"});
        Assert.AreEqual(SomeFunctions.SomeFunctions.Map<List<int>>(new List<List<int>>() {new List<int> {1, 2, 3}, new List<int> {4, 5, 6}, new List<int> {7, 8, 9}}, x => x.Select(i => i * 2).ToList()),
    ...
    {
        Assert.AreEqual(SomeFunctions.SomeFunctions.Fold<int>(new List<int>() {1, 2, 3, 4, 5, 6}, 1, (x, y) => x*y),
            720);
        Assert.AreEqual(SomeFunctions.SomeFunctions.Fold<string>(new List<string>() {"ab", "c", "defg", "hijklmn"}, "", (x, y) => x + y),
            "abcdefghijklmn");
        Assert.AreEqual(SomeFunctions.SomeFunctions.Fold<List<int>>(new List<List<int>>() {new List<int> {1, 2, 3}, new List<int> {4, 5, 6}, new List<int> {7, 8, 9}}, new List<int> {},(x, y) => x.Concat(y).ToList()),
